...
def _parse_com_ia(texto, api_key):
    ...
    system_prompt = (
        ...
        "separe em 'nome' (o prato em si, incluindo o modo de preparo quando for parte do "
        "nome, ex.: 'Robalo grelhado', 'Costela bovina assada') e 'descricao' (o restante: "
        "molhos, acompanhamentos, guarnições — mantendo a pontuação natural, começando "
        "geralmente com uma preposição como 'ao', 'com', 'no' ou com ' - '). Se um prato não "
        "tiver descrição separada (só o nome), deixe 'descricao' como string vazia. Corrija "
        "erros óbvios de português e digitação, mas NÃO invente ingredientes. Responda "
        "APENAS com um JSON válido, um array de objetos com as chaves 'nome' e 'descricao', "
